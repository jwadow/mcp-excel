...
为妈妈用 ❤️ 制作，作者 [@Jwadow](https://github.com/jwadow)
...
[为什么需要这个](#-为什么需要这个) • [我妈妈的评价](#-我妈妈的评价) • [你的智能体能做什么](#-你的智能体能做什么) • [安装](#%EF%B8%8F-安装与配置) • [💖 赞助](#-支持项目)
...
**本地优先架构**
服务器完全在你的本地计算机上运行。你的 Excel 文件在本地处理，**永远不会离开你的电脑**。

**安全吗？**
*   **本地模型（Ollama、LM Studio）：** 你的数据**永远不会**离开你的设备。100% 隐私保护。
*   **云端模型（OpenRouter、ChatGPT）：** 只有操作的*精确结果*（计数、总和、公式）和元数据（列名）会发送到模型。原始数据保留在你的磁盘上。
...
从俄语翻译。她不是技术人员 - 用一根手指打字，每天工作都用Excel。

> *"平时整理这个表格要一个小时，按类别筛选，复制到不同列，算总数。给它布置了任务，3分钟就全做完了。我检查了一遍，都对。现在什么任务都这样，写一下要什么它就做了。说实话我挺震惊的。大半辈子都是手工做这些，现在电脑自己就能明白我要什么。真的省了好多时间。"*
